...
(function() {
    ...
    const styles = `
        ...
        .annual-report-view {
            display: none;
            font-family: 'Noto Serif SC', 'KaiTi', 'STKaiti', '楷体', serif !important;
            padding: 0 !important;
            margin: 0 !important;
            width: 100% !important;
        }
    ...
    async function startScan(forceFullScan = false) {
        ...
        updateProgress('正在准备...', 'Preparing scan...', 1);
    ...
    /**
     * Render the 2025 Annual Report (年度报告)
     */
    function renderAnnualReport(year = 2025, username = '') {
        ...
        if (hasCreatorData) {
            ...
            T = (S / 27000).toFixed(1); // Books equivalent (老人与海)
            ...
            B = c.works; // # works
            C = c.words; // # words
            D = (C / 60000).toFixed(1); // Books equivalent (朝花夕拾)
            ...
            <div class="report-section creator" style="flex: 1;">
                ...
                <div class="report-text">
                    <span class="report-emoji">❤️</span> ${F > 0 
                        ? `你收到了 <span class="nowrap"><span class="report-number">${E.toLocaleString()}</span> 个Kudos</span>，<span class="nowrap"><span class="report-number">${F.toLocaleString()}</span> 条评论</span>`
                        : `你收到了 <span class="nowrap"><span class="report-number">${E.toLocaleString()}</span> 个Kudos</span>`}
                </div>
                ` : ''}
                
                ${U !== '—' ? `
                <div class="report-text">
                    <span class="report-emoji">🏆</span> 你最受欢迎的作品是《<span class="report-highlight pink">${escapeHtml(U)}</span>》，这个故事被翻开过 <span class="nowrap"><span class="report-number">${V.toLocaleString()}</span> 次</span>，${X > 0 
                        ? `<span class="nowrap">有 <span class="report-highlight">${W}</span> 个人留下了Kudos</span>，收获了 <span class="nowrap"><span class="report-highlight">${X}</span> 条评论</span>`
                        : `<span class="nowrap">有 <span class="report-highlight">${W}</span> 个人都很热爱你的作品，留下了Kudos</span>`}
                </div>
                ` : ''}
        ...
        ` : `
            ...
            <div class="report-section reader" style="flex: 1;">
                ...
                <div class="report-text">
                    <span class="report-emoji">🚗</span> 你点击了 <span class="nowrap"><span class="report-number">${M.toLocaleString()}</span> 次</span><span style="font-weight:700; color:#000; font-size:1.1em;">"Yes, Continue"</span>${M >= 600 ? `，足够填满 <span class="nowrap"><span class="report-highlight">${N}</span> 个</span>上海浦东停车场！` : `！`}
                </div>
                ...
                <div class="report-text" style="text-align: center; font-size: 17px;">
                    <span class="report-emoji">💪</span> 女人，就是要多看这些才有力气讨生活啊 <span class="report-emoji">💪</span>
                </div>
            </div>
            <div class="report-footer" style="margin-top: auto;">
                <div class="report-decoration">✦ ✦ ✦</div>
    ...
    async function exportJourney(exportBtn) {
        ...
        try {
            ...
            if (document.querySelector('.report-confirm-view.active')) {
                alert('请先点击 "Yes, Continue" 进入报告页面后再导出\nPlease click "Yes, Continue" first to view the report');
                return;
            }
    ...
    function createUI() {
        ...
        refreshBtn.addEventListener('click', async () => {
            ...
            // Show confirmation
            if (!confirm('This will refresh all data and may take several minutes.\n确定要刷新所有数据吗？这可能需要几分钟时间。')) {
                return;
            }
